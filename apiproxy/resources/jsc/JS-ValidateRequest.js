...
const claimEmail = context.getVariable("jwt.JWT-Verify-JWT-1.decoded.claim.email");
const claimIdcdc = context.getVariable("jwt.JWT-Verify-JWT-1.claim.idcdc"); 
...
function isKeyValid(keys, request) {
    ...
    if(path === "/cargo" && bodyRequest.idCdc){
    ...
    if (idcdc && email) {
        isValidRequest = (idcdc === claimIdcdc && email === claimEmail);
    } 
    ...
    print("Aqui es isValidRequest : ", isValidRequest);
